...
- полный набор интеграционных тестов PostgreSQL и проверка нескольких экземпляров worker остаются отдельной задачей;
- постоянный журнал Telegram Update еще не реализован, поэтому защита от повторной обработки обновлений после перезапуска остается техническим долгом TD-002;
- snooze напоминания, перенос срока кнопкой и навигация статистики по месяцам запланированы отдельными задачами;
